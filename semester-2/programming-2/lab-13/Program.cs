...
namespace Lab13
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\n\n======Test2======");
            string atomFilename = "atom.json";
            Test2(elements, atomFilename);

            Console.WriteLine("\n\n======Test3======");
            Test3(atomFilename);

            Console.WriteLine("\n\n======Test4======");
            string allAtomsFilename = "allAtoms.json";
            Test4(elements, allAtomsFilename);

            Console.WriteLine("\n\n======Test5======");
            Test5(allAtomsFilename);
        }

        static void WriteJson(List<Atom> _atoms, string _filename)
        {
            string json = JsonSerializer.Serialize(_atoms, new JsonSerializerOptions { WriteIndented = true });
            File.WriteAllText(_filename, json);
        }

        static void ReadJson(string _filename)
        {
        ...
        static void Test1(List<Atom> _elements)
        {
            Console.WriteLine("\nDisplaying all Atoms:\n");
            foreach (Atom atom in _elements)
                Console.WriteLine(atom);
        }

        static void Test2(List<Atom> _elements, string _atomFilename)
        {
            WriteJson(new List<Atom> { _elements[0] }, _atomFilename);
            Console.WriteLine($"\nSerialized the first atom to {_atomFilename}");
        }

        static void Test3(string _atomFilename)
        {
            Console.WriteLine("\nDeserialized the first atom from the file:");
            ReadJson(_atomFilename);
        }

        static void Test4(List<Atom> _elements, string _allAtomsFilename)
        {
            WriteJson(_elements, _allAtomsFilename);
            Console.WriteLine($"\nSerialized the entire collection to {_allAtomsFilename}");
        }

        static void Test5(string _allAtomsFilename)
        {
            Console.WriteLine("\nDeserialized all atoms from the file:\n");
            ReadJson(_allAtomsFilename);
        }
    }
    ...
    public class Atom
    {
        ...
        public Atom(string _name, int _proton, int _neutron, double _weight, string _symbol)
        {
            this.Name = _name;
            this.Proton = _proton;
            this.Neutron = _neutron;
            this.Weight = _weight;
            this.Symbol = _symbol;
        }
        ...
        public static Atom Parse(string _line)
        {
            string[] parts = _line.Split(' ');
            if (parts.Length != 5)
            {
                throw new ArgumentException("String must have five parts");
            }
            ...
            {
                throw new FormatException("Failed to parse Atom object", _e);
            }
        }

        public override string ToString()
        {
            return $"{Name} ({Symbol}): Protons = {Proton}, Neutrons = {Neutron}, Weight = {Weight:F4}";
        }
    }
}
